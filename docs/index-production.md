...
> ⚠️ **重要**：OpenClaw 隔离网段建议使用与 VPC 主网段不同的地址空间（如 `10.8.x.0/24`），如VPC具备主网段和附加网段，请使用附加网段对应的VSW，便于 TrafficPolicy 和安全组规则精细化控制。模版会自动从交换机网段 1 推算 VPC 附加 CIDR（取前两段拼 `.0.0/16`），无需手动配置。
