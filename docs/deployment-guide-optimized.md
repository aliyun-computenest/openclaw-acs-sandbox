...
1. [提交工单](https://workorder.console.aliyun.com/) 申请开通镜像缓存功能
...
3. 点击「创建镜像缓存」，配置：
   ...
   - **镜像**：填写 OpenClaw 镜像地址，如 `registry-cn-hangzhou.ack.aliyuncs.com/ack-demo/openclaw:2026.3.2`
...
部署完成后，可通过以下方式验证镜像缓存是否生效：

```bash
# 查看 Pod 事件，检查是否使用了镜像缓存
kubectl describe pod <openclaw-pod-name> | grep -i image

# 如果看到类似 "Successfully pulled image using image cache" 的信息，说明缓存生效
```

> **注意**：如果未开通镜像缓存白名单，Pod 启动时会报 403 错误。请确保先完成白名单申请。
...
# 修改 spec.replicas 增加预热池大小

# 方式二：配置镜像缓存（强烈推荐）
# 镜像缓存需要在 ACS 集群层面申请白名单开通
# 详见文档「配置镜像缓存加速」章节
```

> **注意**：镜像缓存功能需要提前申请开通，无法通过 ROS 模板自动配置。如果 Pod 启动时报 403 错误，说明镜像缓存白名单未开通。
